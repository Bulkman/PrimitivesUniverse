	...
	private void OptimizeMeshes()
	{
		for(int i = 0; i < groups.Length; i++)
		{
			...
			for(int j = 1; j < meshFilters.Length; j++) {
				...
				meshFilters[j].renderer.enabled = false;
				...
				{
					Destroy(meshFilters[j].renderer);
			...
			gmr.castShadows = pool[i].renderer.castShadows;
			gmr.receiveShadows = pool[i].renderer.receiveShadows;
			gmr.useLightProbes = pool[i].renderer.useLightProbes;
			gmr.sharedMaterial = new Material(pool[i].renderer.sharedMaterial);
		}
	}
